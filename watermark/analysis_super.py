...
def get_input_from_file(file_path, line_number):
    """
    Read the 'input' value from a specific line in a JSON-formatted text file.
    ...
    Returns:
        str: The 'input' value of the requested line. Returns None if line number is invalid or parsing fails.
    """
    try:
...
# 第一次打开文件用 'w' 模式，清空内容
"""
with open('scores_super.txt', 'w', encoding='utf-8') as f:
    pass  # 只是为了清空文件
with open('scores_with_text_super.txt', 'w', encoding='utf-8') as f:
    pass  # 只是为了清空文件

# 然后开始循环，每次都用 'a' 模式追加
"""

with open('scores_super.txt', 'a', encoding='utf-8') as f, \
     open('scores_with_text_super.txt', 'a', encoding='utf-8') as f2:
    for i in range(1,1501):
        input = get_input_from_file('new_prompt.txt',i)
        print(input)

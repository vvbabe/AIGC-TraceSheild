...
# 解析数据并合并
list_a = parse_txt_file("deepseek英文/scores.txt")
list_b = []
list_c = list_a + list_b

# 计算 TPR 并打印详细结果（a=0 是下界，b=100 是上界）
stats = calculate_tpr(list_c, k=0.5, a=0, b=100)
print("统计结果:")
print(f"- Token 范围: {stats['token_range'][0]} ~ {stats['token_range'][1]}")
print(f"- 总样本数: {stats['total_samples']}")
print(f"- 真阳性数 (score ≥ {stats['threshold']}): {stats['true_positives']}")
print(f"- 真阳性率 (TPR): {stats['tpr']:.4f}")